...
### 4.7 换访问口令：只有一个按钮，不用旧口令，也不能手输

插件自己的访问口令（链接里的 `?k=`）是**机器生成的凭据**，面板里只有一件事可以做：

1. 点 **「换一个新口令」** → **运行 Harness 的这台机器**生成一把全新的并自己校验（32 位 URL 安全字符，
   约 192 位熵）；你的服务器只负责转发，看不到也存不到它；
2. 代次 +1，**旧链接、旧二维码、已经登录的浏览器立刻全部失效**，对方要拿新链接重进；
3. 面板的确认框里写明上面这条，换完把新口令**只显示一次**（复制进密码管理器，或直接转发新链接）；
4. 每次换口令写一条 `$DSH_HOME/remote-connect/audit.log`（只有时间与事件，**绝不含口令**）。

**为什么删掉了"手输新口令"和"先验旧口令"**：让人自拟口令的结局通常是 `dsh123456`；而"我忘了当前口令"
又必然要留一条绕过验证的后门。两条都不必要 —— 口令是发出去的链接里的一段，用户本来就不该记它。
真正的门槛是"只有宿主窗口能调这个接口"（`requireLocalControl`，服务器侧同样校验），
加上一次显式确认（防手滑，也防被诱导的 POST）。

**接口变化**：`POST /access-key/verify`（校验旧口令）**已删除**，现在返回 404。
`POST /access-key/rotate` 仍然存在，但请求体只认 `{"acknowledge": true}`：
**任何 `password` / `current` / `confirm` / `reset` 字段都会被忽略** —— 口令只能由本机生成。

### 4.8 口令在全机范围内唯一，一把口令只对应一个 Harness

这台机器上不止一把口令：本机访问口令 + 每个租户各一把（多租户见 [`multi-tenant.zh.md`](multi-tenant.zh.md)）。
两条不变量由 `lib/core/keypool.js` 在**生成之前**保证，账本落在
`$DSH_HOME/remote-connect/keys-used.json`（权限 0600，原子写入）：

1. **任意两把口令都不相同** —— 包括已经换掉、退休的旧值（它不可能再被发给任何人）；
2. **一把口令只属于一个上游**：本机口令只通向本机 Harness，某个租户的口令只通向他自己那个实例。
   多租户网关下本机口令**打不开**任何租户的 Harness（`test/verify.mjs` 里有回归断言）。

账本里只存 **sha256 前 16 位指纹**，绝不存明文；撞上已用过的值就**重新生成**（生成时重试），
而不是等你提交之后才报错。手工编辑注册表也绕不过去：`tenants.json` 里重复的密钥、或等于本机口令的值，
会被逐条报告并跳过。
