...
export function loadAccessKeyState(file, seed) {
  ...
  return { file, accessKey: seeded, epoch: 0, updatedAt: null, generated: seed === '' }
}
...
export function apply(ctx, rawConfig) {
  ...
  function rotateAccessKey(next) {
    ...
    accessKey = value
    keyEpoch += 1
    saveAccessKeyState(keyState.file, { accessKey, epoch: keyEpoch, updatedAt: new Date().toISOString() })
    ...
    try {
      ...
      if (route === '/access-key/rotate') {
        ...
        try {
          ...
          rotated = rotateAccessKey(typeof body.password === 'string' && body.password !== '' ? body.password : '')
        } catch (error) {
          lastError = String(error?.message ?? error)
          throw error
        } finally {
