...
import { generateAccessKey } from './core/tenant.js'
...
export function apply(ctx, rawConfig) {
  ...
  // 先声明，再装配（口令池的 reserved() 会惰性读它们，避免初始化环 / 暂时性死区）
  let accessKey = ''
  let tenancy = null
  /**
   * 口令池：**全机唯一**的口令命名空间（本机访问口令 + 每个租户的口令，
   * 连同换掉过的旧值），只存 sha256 前 16 位指纹，不存明文。
   * 详见 core/keypool.js；活跃值由 reserved() 惰性提供。
   */
  ...
  // 没有配置种子时，用口令池发一把新的（保证不与任何历史/活跃口令重复）
  const keySeed = config.public.accessKey !== '' ? config.public.accessKey : keyPool.issue().value
  const keyState = loadAccessKeyState(path.join(pluginStateDir(), 'access-key.json'), keySeed)
  ...
  function rotateAccessKey(next) {
    ...
    saveAccessKeyState(keyState.file, {
      ...
      // 兼容字段：镜像口令池最近 50 条指纹（真值在 keys-red.json）
  ...
  // （tenancy 在文件上方已声明：keyPool.reserved() 会在装配阶段惰性读它）
