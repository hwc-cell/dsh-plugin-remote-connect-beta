...
/**
 * 插件自己的状态目录（0600）：租户注册表、Cookie 签名密钥、生成的访问密钥都落在这里。
 * 默认跟 harness home 走，避免把状态写进用户主目录根下。
 */
export function pluginStateDir(env = process.env) {
  const home = typeof env.DSH_HOME === 'string' && env.DSH_HOME !== '' ? env.DSH_HOME : path.join(os.homedir(), '.dsh')
...
/**
 * Cookie 签名密钥：多租户下必须独立于任何租户的 accessKey，
 * 否则轮换某个租户的密钥会连带把别人的登录态弄失效。
 * 首次生成后落盘（0600），重启后沿用（否则重启即全员掉线）。
 */
function loadGateSecret(file) {
  try {
  ...
  try {
    ...
    logOnce('Cookie 签名密钥落盘失败（重启后所有会话会重新登录）：' + String(error?.message ?? error))
  }
  return secret
}
...
export function apply(ctx, rawConfig) {
  ...
  const gateSecretFile = path.join(pluginStateDir(), 'gate.secret')
  const gateSecret = loadGateSecret(gateSecretFile)
  ...
  async function startLan() {
    ...
    lanProxy = createProxy({
      ...
      listenHost: '0.0.0.0',
      // 多租户：不再有"全局密钥"，每个请求按自己的 ?k=/Cookie 决定去哪个租户
      accessKey: '',
      gateSecret,
