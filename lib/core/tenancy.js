 ...
 * @param {object} [options.keyPool] 口令池：租户密钥必须在全机范围内唯一（见 core/keypool.js）
 */
export function createTenancy(options) {
  ...
  /**
   * 面板/CLI 新增租户：id 可由显示名自动收敛，访问密钥由注册表（口令池）自动生成，
   * 保证**全机唯一**：不会撞上本机访问口令，也不会复用任何发过又换掉的旧口令。
  ...
  function add(input = {}) {
    ...
    const tenant = registry.add({
      ...
      // 留空 = 交给注册表生成（走口令池）；显式给值的话注册表会照样查重
      accessKey: typeof input.accessKey === 'string' && input.accessKey !== '' ? input.accessKey : '',
