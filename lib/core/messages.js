...
const en = {
  ...
  'cli.help': [
    ...
    '  --edge-auth              emit the optional edge password gate (off by default; see SECURITY.md)',
...
const zh = {
  ...
  'preflight.tunnel.notKept.hint.listen':
    '远端口 {remotePort} 拨不上，但本机已经没有隧道进程了 —— 最可能是**服务器侧那个端口还被上一次的会话占着没回收**（ssh 客户端已消失、sshd 还没发现）。通常 1–2 分钟内随 ClientAliveInterval 自动释放，重试即可；持续超过 2 分钟请让服务器侧执行 pkill -f \'sshd.*dshtunnel\'（或 systemctl restart sshd）。次要可能：如果你改过远端端口，检查 authorized_keys 的 permitlisten 是否恰好是 127.0.0.1:{remotePort}。这期间公网入口会返回 502（边缘口令门仍正常返回 401）',
  ...
  'cli.help': [
    ...
    '  --edge-auth              生成边缘口令门（默认不生成；理由见 SECURITY.md）',
