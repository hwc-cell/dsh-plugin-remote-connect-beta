...
const en = {
  ...
  'preflight.tls.uncovered.hint': 'Extend your existing certificate to include this domain, or use a covered one. Note: if you just disabled this domain\'s site (no server block for it in nginx sites-enabled), the request lands on the 443 default server and gets that server\'s certificate — in that case this line and the ECONNRESET below are both expected',
  ...
  'preflight.tunnel.existing': 'Remote port {remotePort} is already held by a tunnel on this machine (pid {pid}) — the tunnel is running, nothing to start',
  ...
  'preflight.tunnel.notKept.hint.listen':
    'Remote port {remotePort} cannot be bound and there is no tunnel process left on this machine, so the server is most likely still holding that port for a previous session (the ssh client is gone, sshd has not noticed yet). It normally clears within 1-2 minutes (ClientAliveInterval); retry, and if it persists past two minutes ask the server side to run pkill -f \'sshd.*dshtunnel\' (or systemctl restart sshd). Second possibility: if you changed the remote port, check that permitlisten is exactly 127.0.0.1:{remotePort}. While this lasts the public entry answers 502 (the edge gate still answers 401)',
  ...
  'host.error.keyRotateNeedsAck': 'Rotating needs an explicit confirmation (it invalidates every old link and session)',
  ...
  'cli.help': [
    ...
    '  --wait <seconds>             if the remote port is still held by a dead session, wait for it',
    '',
    'snippets options:',
...
const zh = {
  ...
  'preflight.tls.uncovered.hint': '给现有证书扩签该域名，或改用一个被覆盖的域名。注意：如果你刚把该域名的站点停用了（nginx sites-enabled 里没有它的 server block），请求会落到 443 的默认站点上，于是拿到默认站点的证书 —— 这种情况这条以及下面的 ECONNRESET 都是预期现象',
  ...
  'preflight.tunnel.existing': '远端口 {remotePort} 已被本机的一条隧道占用（pid {pid}）—— 隧道正在运行，无需重复启动',
  'preflight.tunnel.existing.hint': '这条就是正在提供服务的隧道。想真的测一次重连，就先把它停掉（插件会自动重开一条）再检查',
  'preflight.tunnel.notKept.hint.listen':
    '远端口 {remotePort} 拨不上，但本机已经没有隧道进程了 —— 最可能是服务器侧那个端口还被上一次的会话占着没回收（ssh 客户端已消失、sshd 还没发现）。通常 1–2 分钟内随 ClientAliveInterval 自动释放，重试即可；持续超过 2 分钟请让服务器侧执行 pkill -f \'sshd.*dshtunnel\'（或 systemctl restart sshd）。次要可能：如果你改过远端端口，检查 authorized_keys 的 permitlisten 是否恰好是 127.0.0.1:{remotePort}。这期间公网入口会返回 502（边缘口令门仍正常返回 401）',
  ...
  'preflight.tailscale.failed.hint.loggedout': '先运行 tailscale up 登录你的 tailnet',
  'preflight.tailscale.failed.hint.disabled': '到 Tailscale 管理后台为这个节点启用 Funnel（需要打开 HTTPS 证书）',
  'preflight.tailscale.failed.hint.generic': '在运行 Harness 的这台机器上执行 tailscale funnel status，看完整输出',
  ...
  'host.error.keyRotateNeedsAck': '换新口令需要显式确认（旧链接与旧会话会全部失效）',
  ...
  'cli.help': [
    ...
    '  --wait <秒>              远端口被上一次的死会话占着时，自动等它释放再判定',
    '',
    'snippets 选项：',
