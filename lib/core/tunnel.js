 ...
 * 下一次重连要等多久。
 *
 * 很多服务器会对 SSH 端口加**新连接频率限制**（例如每 IP 60 秒 20 次，fail2ban 与防火墙
 * 规则里都常见）。固定 2 秒重试一分钟就是 30 次，会直接撞上限流，而限流表现出来只是
 * "ssh 超时"，日志里看不出原因 ——
...
export function createTunnel(options) {
  ...
  // 重连退避：服务器侧若对 SSH 新连接限流（例：20 次/60s），固定间隔重试会撞上它
  ...
  function launchProcess() {
    ...
    spawned.on('exit', (code, signal) => {
      ...
      // 变成"固定间隔猛重试"，正好撞上服务器 SSH 端口的新连接限流。
