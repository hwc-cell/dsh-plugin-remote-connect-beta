...
export async function checkSshTunnel(options) {
  ...
  return await new Promise((resolve) => {
    ...
    child.on('exit', async (code) => {
      ...
      if (code === 0) return
      // 远端口被服务器上"上一次的会话"占着时，等一会儿会自动释放；
      // --wait <秒> 让调用方不必手动反复重试（默认 0 = 立即返回失败）。
      const waitMs = Number(options.waitMs ?? 0)
      if (waitMs > 0 && text.includes('remote port forwarding failed') && Date.now() - startedAt < waitMs) {
        setTimeout(() => {
          void checkSshTunnel({ ...options, waitMs }).then(finish)
        }, 15000)
        return
      }
