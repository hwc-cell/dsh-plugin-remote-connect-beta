...
export function createProxy(options = {}) {
   ...
   * 密钥无效/过期时给人看的页面。
   *
   * 只对"确实带着 ?k= 或网关 Cookie 来敲门"的请求返回：这种人已经知道入口存在，
   * 给他一句人话比裸 404 有用；而没有任何凭据的扫描器仍然只看到裸 404（不暴露存在性）。
   * 页面里不放任何敏感信息，也不回显他试过的密钥。
   */
  function gatePage(res, req) {
    const wanted = new URL(req.url ?? '/', 'http://placeholder').searchParams.has('k')
    const hasCookie = readCookie(req.headers.cookie, GATE_COOKIE) !== undefined
    if (!wanted && !hasCookie) {
      notFound(res)
      return
    }
    ...
    const body = Buffer.from(
      '<!doctype html><meta charset="utf-8"><title>' +
        t('gate.invalid.title') +
        '</title><style>body{font:15px/1.7 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;margin:0;padding:48px 20px;background:#f6f7f9;color:#18191c}main{max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:24px 28px}h1{font-size:17px;margin:0 0 12px}code{background:#f0f1f3;padding:1px 5px;border-radius:4px}</style><main><h1>' +
        t('gate.invalid.title') +
        '</h1><p>' +
        t('gate.invalid.body') +
        '</p><p>' +
   ...
   */
  function gate(req) {
    if (!multiTenant && config.accessKey === '') return { status: 'off' }
    ...
    if (typeof raw === 'string' && raw.includes('.')) {
      ...
      if (safeEqual(mac, sign(payload, config.gateSecret))) {
        ...
        const tenantId = separator === -1 ? undefined : payload.slice(separator + 1)
    ...
    if (typeof query === 'string' && query !== '') {
      ...
      } else if (config.accessKey !== '' && safeEqual(query, config.accessKey)) {
        return { status: 'grant' }
      }
    }
    return { status: 'deny' }
  ...
  function grantCookie(req, res, tenantId) {
    const payload =
      String(Date.now() + config.gateTtlHours * 3600 * 1000) + (tenantId === undefined ? '' : ':' + tenantId)
  ...
  /** 令牌可能过期（Harness 重启后日志里是新令牌）：丢掉缓存重新发现。 */
  ...
  function handleRequest(req, res) {
    ...
    const upstream = http.request(
      ...
      (up) => {
        ...
        if (up.statusCode === 401 && isIndex && retried === false) {
          retried = true
          up.resume()
          ...
          if (fresh !== '') {
            config.log('浏览器会话已失效，改用启动令牌重新登录')
            res.writeHead(303, {
              location: '/?token=' + encodeURIComponent(fresh),
              'cache-control': 'no-store',
            })
            res.end()
            return
  ...
  function handleUpgrade(req, socket, head) {
    ...
    const decision = gate(req)
    const route = decision.status === 'ok' ? resolveRoute(decision.tenantId) : null
  ...
  function info() {
    ...
    return {
      ...
      lanUrl,
      gate: multiTenant || config.accessKey !== '',
