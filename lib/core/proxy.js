...
export function createProxy(options = {}) {
  ...
  /**
   * 这个值"像不像我们自己发出去的口令"？用来区分两种失败：
   *   - 像 → `key-unusable`："这条链接已经失效（多半是换过口令），去面板重新复制"
   *   - 不像 → `bad-key`："抄错了 / 被聊天软件截断了"
   * 两种补救动作完全不同，所以别混。
   *
   * 形态要跟着**生成器**走：现在发出去的是 32 位 base64url（generateAccessKey，192 bit），
   * 历史上还发过 32 位十六进制。两种都算"像"，否则用户拿着一条真失效的链接会被判成"抄错了"。
   */
  function looksLikeKey(value) {
    const text = String(value ?? '')
    return /^[0-9a-f]{32}$/i.test(text) || /^[A-Za-z0-9_-]{24,}$/.test(text)
  }
  ...
  function gate(req) {
    ...
    if (typeof query === 'string' && query !== '') {
      ...
      // 形如"我们发出去的 key"（32 字符 base64url；β3 之前是 32 位十六进制）但不是当前值
      // → 多半是轮换过的旧链接；其它形态（短串、手输错）→ 直接算"密钥不正确"。两者要能区分，现场才不用猜。
      return { status: 'deny', reason: looksLikeKey(query) ? 'key-unusable' : 'bad-key', presented: query }
    }
  ...
  function grantCookie(req, res, tenantId) {
    ...
    // Secure 只在这条请求确实走了 HTTPS 时加：公网入口前面永远是 nginx/Caddy 这类 TLS 代理
    //（生成的反代片段都会带 X-Forwarded-Proto），少了它，用户用 http:// 打开入口时
    // 浏览器会把这道门的 Cookie 明文发一次（80 端口那次请求发生在 301 跳转之前）。
    // 不能无条件加：局域网入口是裸 http（8787），加死会让局域网访问每次都退回 ?k= 链接。
