...
export function createProxy(options = {}) {
   ...
   * 是否给这次请求注入启动令牌。
   *
   * 只看"是不是首页 GET"，**不看**浏览器有没有 Cookie —— 这里踩过坑：
   * 浏览器留着上一次 Harness 的 `dsh-auth-*` Cookie 时，旧逻辑就不再注入令牌，
   * 而 Harness 重启后那个 Cookie 已经失效 → 页面能开但会话列表永远是空的
   * （"暂无会话"），而且刷新也没用，除非手动清 Cookie。
   * 首页注入令牌 = 顺手把失效的浏览器会话换成新的，这是自愈。
   */
