...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      'key.hint': '口令由运行 Harness 的这台机器生成并校验（32 位 URL 安全字符，约 192 位熵）：它就是链接里的 ?k=。你的服务器只负责转发请求，看不到也存不到它。换它不用动服务器，你也不需要记住它。',
    ...
    const en = {
      ...
      'key.hint': 'This password is generated and checked by the machine running the Harness (32 URL-safe characters, ~192 bits): it is the ?k= in your link. Your server only forwards requests — it never sees or stores it. Rotating needs no server change, and you never have to remember it.',
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            state.revealedKey !== null
              ...
              : React.createElement('div', { className: 'dshRcDesc' }, t('key.hint')),
