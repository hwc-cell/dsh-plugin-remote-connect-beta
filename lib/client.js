 ...
 *
 * @module dsh-plugin-remote-connect/client
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      'qr.hint': '扫码即可在手机上打开',
      'key.customPlaceholder': '也可以自己设：输入新口令（≥12 位）',
      'key.setCustom': '设为新口令',
      'key.customHint': '自定义口令会当场校验强度（太短/常见弱口令会被拒绝并说明原因）。',
      'key.label': '访问口令',
      'key.reveal': '显示',
      'key.hide': '收起',
      'key.rotate': '生成新的',
    ...
    const en = {
      ...
      'key.customPlaceholder': 'Or set your own: new password (12+ chars)',
      'key.setCustom': 'Set as new password',
      'key.customHint': 'A custom password is checked on the spot (too short or too common is refused with the reason).',
    ...
    const PLUGIN_ID = 'dsh-plugin-remote-connect'
    const POLL_MS = 5000
    ...
    /** 生成新的访问口令：旧链接与旧 Cookie 立即失效，二维码随 /state 自动更新。 */
    async function rotateKey() {
      if (typeof window !== 'undefined' && typeof window.confirm === 'function') {
        if (window.confirm(t('key.rotateConfirm')) !== true) return
      }
    ...
    /** 用用户自己输入的口令轮换（强度由宿主半校验，失败会带原因回来）。 */
    async function setCustomKey() {
      const password = String(store.keyDraft ?? '').trim()
      if (password === '') return
      setState({ keyBusy: true, error: null })
      const payload = await api('/access-key/rotate', {
        method: 'POST',
      ...
      setState({
        keyBusy: false,
        keyDraft: payload.ok === false ? store.keyDraft : '',
        revealedKey: typeof payload.accessKey === 'string' ? payload.accessKey : null,
        error: payload.ok === false ? payload.error : null,
      })
    }
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement('input', {
                ...
                type: 'text',
                placeholder: t('key.customPlaceholder'),
                value: state.keyDraft ?? '',
                onChange: (event) => setState({ keyDraft: event.target.value }),
                onKeyDown: (event) => {
                  if (event.key === 'Enter') void setCustomKey()
                },
              }),
              React.createElement(
                'button',
                {
                  type: 'button',
                  className: 'dshRcButton isGhost',
                  disabled: state.busy === true || state.keyBusy === true || !canControl || String(state.keyDraft ?? '').trim() === '',
                  onClick: () => void setCustomKey(),
                },
                t('key.setCustom'),
              ),
            ),
            state.revealedKey !== null
              ...
              : React.createElement('div', { className: 'dshRcDesc' }, t('key.customHint')),
