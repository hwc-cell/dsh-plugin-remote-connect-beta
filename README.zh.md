...
如果一台机器确实要给多个人用（家庭服务器、共用工作站），插件带一层多租户：每个租户一个**独立**
Harness 进程，各自的 `DSH_HOME`、端口、启动令牌，网关按访问密钥把人送进各自实例。这一层
**默认关闭**，取舍写在 [`docs/multi-tenant.zh.md`](docs/multi-tenant.zh.md) 里。

---

## 风险 —— 开放之前请先读这一节

**你放到网络上的是什么。** Harness 不是网页：它会以启动它的那个用户的身份执行命令、读写文件。
任何越过下面这几道门的人，拿到的就是这台机器上的这个权限。
...
开放之前请把 root/管理员私钥挪走，至少加口令并交给 agent 管理。

**沙箱是 DSH 的，不是本插件的。** 访客继承的是你这台 Harness 的权限档位。如果你的档位是
`danger-full-access`，访客就拥有你拥有的一切。对外开放时请用 `workspace-write`（或更窄的档位）；
本插件自己不做任何沙箱限制。

**口令从哪来、存在哪。** 访问口令由**运行 Harness 的那台机器**生成并校验 —— 你的服务器只负责转发，
看不到也存不到它。它、它的指纹历史、审计日志、Cookie 签名密钥都在
`$DSH_HOME/remote-connect/`，权限 0600。访问密钥**没有**存进 macOS 钥匙串（已知限制）。
密钥换来的浏览器会话是 12 小时；轮换或重置密钥会让已发出的所有 Cookie 立即失效。

**可用性也是会丢的东西。** 轮换密钥会让所有链接与会话失效 —— 这是设计目的，但记得通知持有链接的人。
局域网地址会随网络变化。隧道断开时公网入口返回 503 并带 `X-DSH-Reason: tunnel-down`。
对 SSH 连接限流的主机会让重连表现为**静默超时**，本插件因此采用指数退避。

**什么人不该用它。** 任何会把 shell 交给陌生人、而机器上还存着别的凭据的人。
"别人也想要远程访问"不是把你的给他用的理由 —— 那是他自己装一份的理由。

### 开放前的六项检查

1. 对外开放的这个 Harness **不要**用 `danger-full-access` 档位。
2. 这台机器上不要放 root/管理员 SSH 私钥或保存的服务器密码（或至少加口令）。
3. 边缘口令用 `dsh-remote credential` **生成**（别自己想），并存进密码管理器。
4. `?k=` 链接只发给你愿意让他直接坐到这台电脑前的人。
5. 除非你信任当前网络，否则关掉局域网入口。
6. 开启后立刻打开 `https://<你的域名>/_dsh/health`，确认 `tunnel: up`，且 `key_fp8` 与面板上的指纹一致。

### 急停开关

- 面板点「**停止**」：公网监听与隧道立刻停。
- 或在 `cordis.patch.yml` 里把 `public.enabled` / `lan.enabled` 改成 `false`，重启 DSH。
- 轮换访问密钥：所有已有链接与已发出的会话一次性失效。
...
>
> **访问口令只有两个动作：看、以及点「换一个新口令」。** 没有"手输新口令"，也不用先输入旧口令：
> 口令由**运行 Harness 的那台机器**生成并校验（32 位随机串，约 192 位熵）—— 你的服务器只负责转发，看不到也存不到它。
> 点一下就换掉，旧链接/旧二维码/已登录的浏览器立刻全部失效，
> 新口令只显示一次。全机（本机口令 + 每个租户的口令）两两不重复，连**换掉过的旧值**也不会再发出去 —— 见
> [`docs/self-host.zh.md` §4.7/§4.8](docs/self-host.zh.md)。
...
lib/core/keypool.js      口令池：本机唯一的口令命名空间（谁都不能撞谁，退休的旧值不再发）
